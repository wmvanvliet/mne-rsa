...
rdms = [mne_rsa.compute_rdm(metadata[col], metric='euclidean')
        for col in columns]
...
rsa_results = mne_rsa.rsa(generate_eeg_rdms(), rdms, metric='spearman',
                          verbose=True, n_data_rdms=n_times, n_jobs=1)
