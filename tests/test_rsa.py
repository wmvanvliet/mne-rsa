...
class TestRSAGen:
    ...
    def test_regression(self):
        ...
        rsa_val = next(rsa_gen(data_rdm, [model_rdm1, model_rdm2],
                               metric='regression'))
...
class TestRSA:
    ...
    def test_progress_bar(self):
        ...
        assert rsa([rdm()], rdm(), verbose=True) == [1.]
        assert rsa([rdm()], rdm(), verbose=True, n_data_rdms=1) == [1.]
        assert rsa(rdm_gen([rdm()]), rdm(), verbose=True) == [1.]

    def test_parallel_processing(self):
        """Test running rsa with multiple cores."""
        assert_equal(rsa([rdm(), rdm()], rdm(), n_jobs=2), [1., 1.])
...
class TestRSASearchlight:
    ...
    def test_rsa_temp(self):
        ...
        rsa_result = rsa_array(data, model_rdm, patches,
        ...
        patches = searchlight(data.shape, temporal_radius=1)
        rsa_result = rsa_array(data, [model_rdm, model_rdm], patches,
                               data_rdm_metric='euclidean')
        ...
        patches = searchlight(data.shape, temporal_radius=1)
        rsa_result = rsa_array(data, model_rdm, patches,
                               data_rdm_metric='euclidean', n_jobs=2)
        assert rsa_result.shape == (2,)
        assert_equal(rsa_result, 1)
